...
# --- BLOQUE 5: Verificar la referencia combinada ---

# Releemos el FASTA combinado y comprobamos su contenido
ref <- readDNAStringSet(output_fasta)

cat("Resumen de la referencia combinada:\n")
cat("  - Total de secuencias:", length(ref), "\n")
cat("  - Longitud mínima:    ", min(width(ref)), "pb\n")
cat("  - Longitud máxima:    ", max(width(ref)), "pb\n")
cat("  - Longitud mediana:   ", median(width(ref)), "pb\n")

# Mostrar los IDs de los primeros 5 transcritos para ver el formato
cat("\nPrimeros 5 identificadores:\n")
print(head(names(ref), 5))

# Comprobar que los IDs coinciden con los del TSV

ids_fasta <- sub(" .*$", "", names(ref))
cat("\n¿Cuántos IDs del FASTA están también en el TSV?\n")
en_tsv <- ids_fasta %in% tx2gene$transcript_id
cat("  Coinciden:", sum(en_tsv), "de", length(ids_fasta), "\n")

if (!all(en_tsv)) {
  cat("\n⚠ IDs del FASTA que NO están en el TSV (primeros 10):\n")
  print(head(ids_fasta[!en_tsv], 10))
}

# --- BLOQUE 6: Construir el índice de Rsubread ---
#BiocManager::install("Rsubread")
#BiocManager::install("Rsubread", ask = FALSE, update = FALSE)
library(Rsubread)

# Carpeta del indice
indice_dir <- "02_alineamiento/indice"
dir.create(indice_dir, showWarnings = FALSE)

# Prefijo del índice: Rsubread generará varios archivos que comparten

indice_prefijo <- file.path(indice_dir, "ref_obesidad")

# Construir el índice
# - basename:    prefijo de los archivos de salida
# - reference:   archivo FASTA de entrada
# - gappedIndex: FALSE → índice completo en memoria (rápido, recomendado
#                para referencias pequeñas como la nuestra, ~5 MB)
# - indexSplit:  FALSE → no dividir el índice en trozos (solo necesario
#                para genomas grandes tipo humano completo)
buildindex(
  basename     = indice_prefijo,
  reference    = output_fasta,
  gappedIndex  = FALSE,
  indexSplit   = FALSE
)

cat("\n✔ Índice construido en:", indice_dir, "\n")
list.files(indice_dir)
